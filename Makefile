...
CXXFLAGS += -I$(SYS_INCLUDE_DIR) -I$(LIB_HEADER_DIR) -I$(SYS_INCLUDE_DIR)/opencv4       # for two posible places for headers
CXXFLAGS += $(OPENCV_CXXFLAGS) $(PYTHON_CXXFLAGS)

# linking params
LINK_FLAGS := -O3 -shared -std=gnu++11
LINK_FLAGS += $(OPENCV_LIBS) $(PYTHON_LIBS)
LINK_FLAGS += -lopencv_core -lopencv_imgproc -lopencv_imgcodecs -lopencv_videoio -lopencv_highgui \
...
LINK_FLAGS += -lopencv_dnn -lopencv_text -lopencv_tracking
